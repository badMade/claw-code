...
def build_parser() -> argparse.ArgumentParser:
    ...
    summary_parser = subparsers.add_parser('summary', help='render a Markdown summary of the Python porting workspace')
    summary_parser.set_defaults(func=handle_summary)

    manifest_parser = subparsers.add_parser('manifest', help='print the current Python workspace manifest')
    manifest_parser.set_defaults(func=handle_manifest)

    parity_audit_parser = subparsers.add_parser('parity-audit', help='compare the Python workspace against the local ignored TypeScript archive when available')
    parity_audit_parser.set_defaults(func=handle_parity_audit)

    setup_report_parser = subparsers.add_parser('setup-report', help='render the startup/prefetch setup report')
    setup_report_parser.set_defaults(func=handle_setup_report)

    command_graph_parser = subparsers.add_parser('command-graph', help='show command graph segmentation')
    command_graph_parser.set_defaults(func=handle_command_graph)

    tool_pool_parser = subparsers.add_parser('tool-pool', help='show assembled tool pool with default settings')
    tool_pool_parser.set_defaults(func=handle_tool_pool)

    bootstrap_graph_parser = subparsers.add_parser('bootstrap-graph', help='show the mirrored bootstrap/runtime graph stages')
    bootstrap_graph_parser.set_defaults(func=handle_bootstrap_graph)
    ...
    list_parser.set_defaults(func=handle_subsystems)
    ...
    commands_parser.set_defaults(func=handle_commands)
    ...
    tools_parser.set_defaults(func=handle_tools)
    ...
    bootstrap_parser.set_defaults(func=handle_bootstrap)
    ...
    loop_parser.set_defaults(func=handle_turn_loop)
    ...
    flush_parser.set_defaults(func=handle_flush_transcript)
    ...
    load_session_parser.set_defaults(func=handle_load_session)
    ...
    teleport_parser.set_defaults(func=handle_teleport_mode)
    ...
    direct_parser.set_defaults(func=handle_direct_connect_mode)
    ...
    deep_link_parser.set_defaults(func=handle_deep_link_mode)
    ...
    exec_command_parser.set_defaults(func=handle_exec_command)
    ...
    exec_tool_parser.set_defaults(func=handle_exec_tool)
