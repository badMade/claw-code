...
def handle_summary(args: argparse.Namespace, manifest) -> int:
    print(QueryEnginePort(manifest).render_summary())
    return 0


def handle_manifest(args: argparse.Namespace, manifest) -> int:
    print(manifest.to_markdown())
    return 0


def handle_parity_audit(args: argparse.Namespace, manifest) -> int:
    print(run_parity_audit().to_markdown())
    return 0


def handle_setup_report(args: argparse.Namespace, manifest) -> int:
    print(run_setup().as_markdown())
    return 0


def handle_command_graph(args: argparse.Namespace, manifest) -> int:
    print(build_command_graph().as_markdown())
    return 0


def handle_tool_pool(args: argparse.Namespace, manifest) -> int:
    print(assemble_tool_pool().as_markdown())
    return 0


def handle_bootstrap_graph(args: argparse.Namespace, manifest) -> int:
    print(build_bootstrap_graph().as_markdown())
    return 0


def handle_subsystems(args: argparse.Namespace, manifest) -> int:
    for subsystem in manifest.top_level_modules[: args.limit]:
        print(f"{subsystem.name}\t{subsystem.file_count}\t{subsystem.notes}")
    return 0


def handle_commands(args: argparse.Namespace, manifest) -> int:
    if args.query:
...
def handle_tools(args: argparse.Namespace, manifest) -> int:
    if args.query:
...
def handle_route(args: argparse.Namespace, manifest) -> int:
...
def handle_bootstrap(args: argparse.Namespace, manifest) -> int:
    print(PortRuntime().bootstrap_session(args.prompt, limit=args.limit).as_markdown())
    return 0


def handle_turn_loop(args: argparse.Namespace, manifest) -> int:
...
def handle_flush_transcript(args: argparse.Namespace, manifest) -> int:
...
def handle_load_session(args: argparse.Namespace, manifest) -> int:
...
def handle_remote_mode(args: argparse.Namespace, manifest) -> int:
    print(run_remote_mode(args.target).as_text())
    return 0


def handle_ssh_mode(args: argparse.Namespace, manifest) -> int:
    print(run_ssh_mode(args.target).as_text())
    return 0


def handle_teleport_mode(args: argparse.Namespace, manifest) -> int:
    print(run_teleport_mode(args.target).as_text())
    return 0


def handle_direct_connect_mode(args: argparse.Namespace, manifest) -> int:
    print(run_direct_connect(args.target).as_text())
    return 0


def handle_deep_link_mode(args: argparse.Namespace, manifest) -> int:
    print(run_deep_link(args.target).as_text())
    return 0


def handle_show_command(args: argparse.Namespace, manifest) -> int:
...
def handle_show_tool(args: argparse.Namespace, manifest) -> int:
...
def handle_exec_command(args: argparse.Namespace, manifest) -> int:
    result = execute_command(args.name, args.prompt)
    print(result.message)
    return 0 if result.handled else 1


def handle_exec_tool(args: argparse.Namespace, manifest) -> int:
...
COMMAND_HANDLERS = {
    ...
    "parity-audit": handle_parity_audit,
    "setup-report": handle_setup_report,
    "command-graph": handle_command_graph,
    "tool-pool": handle_tool_pool,
    "bootstrap-graph": handle_bootstrap_graph,
    "subsystems": handle_subsystems,
    "commands": handle_commands,
    "tools": handle_tools,
    "route": handle_route,
    "bootstrap": handle_bootstrap,
    "turn-loop": handle_turn_loop,
    "flush-transcript": handle_flush_transcript,
    "load-session": handle_load_session,
    "remote-mode": handle_remote_mode,
    "ssh-mode": handle_ssh_mode,
    "teleport-mode": handle_teleport_mode,
    "direct-connect-mode": handle_direct_connect_mode,
    "deep-link-mode": handle_deep_link_mode,
    "show-command": handle_show_command,
    "show-tool": handle_show_tool,
    "exec-command": handle_exec_command,
    "exec-tool": handle_exec_tool,
}
